...
class lstm_resnet_bahdanau_input_multiple_layers_hope(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 2, return_attn = False, idx2word = None):
        ...
        self.convnext_tiny = ResNetModel.from_pretrained('microsoft/resnet-18').to(self.DEVICE)
        ...
        self.attention = BahdanauAttention(512, rnn_layers, device=self.DEVICE)
        self.inp_att = nn.Linear(512*2, 512).to(self.DEVICE)
    ...
    def forward(self, img, ground_truth = None):
        ...
        feat = self.convnext_tiny(img) # batch, 512, 1, 1
        img_channels = feat.last_hidden_state # batch, 512, 7, 7
        img_channels = img_channels.view(batch_size, 512, -1) # batch, 768, 49
...
if __name__ == "__main__":
    ...
    # Init model
    model = lstm_resnet_bahdanau_input_multiple_layers_hope(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)
