...
class cross_attention_lstm_efficient_multiple_layers(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 1, return_attn = False, idx2word = None):
        ...
        self.cross_attention = nn.ModuleList([CrossAttention(49, 1536, ret_attn=return_attn).to(self.DEVICE) for i in range(rnn_layers)] )
