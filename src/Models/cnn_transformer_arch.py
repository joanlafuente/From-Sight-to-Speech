...
class cnn_transformer(nn.Module):
    ...
    def forward(self, img, ground_truth = None):
        ...
        img_enc = self.resnet(img).last_hidden_state # batch, 512, 7, 7
        img_enc = img_enc.view(img_enc.shape[0], img_enc.shape[1], -1) # Batch, 512, 49
        img_enc = img_enc.permute(2, 0, 1) # 49, Batch, 512
        ...
        if (self.teacher_forcing_ratio == 0) or (ground_truth is None):      
            for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
                out = self.transformer_decoder(self.positional_encoder(inp), img_enc) # 1, batch, 512
                inp = torch.cat((inp, out[-1:, :, :]), dim=0)
            pred = inp
        else:
            ground_truth = self.embed(ground_truth) # batch, seq, 512
            ground_truth = ground_truth.permute(1, 0, 2) # seq, batch, 512
            ground_truth = self.positional_encoder(ground_truth) # batch, seq, 512
            pred = self.transformer_decoder(ground_truth, img_enc) # seq, batch, 512
...
if __name__ == "__main__":
    ...
    # Init model
    model = cnn_transformer(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)
