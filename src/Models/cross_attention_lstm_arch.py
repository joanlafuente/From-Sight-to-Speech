...
class cross_attention_lstm(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, dropout = 0, rnn_layers = 1):
        ...
        #self.RNN = nn.LSTM(512, 512, num_layers=5)
        self.RNN = nn.LSTM(1024, 512, num_layers = rnn_layers)
        ...
        self.hiddenattention2hidden = nn.Linear(1024, 512)
    ...
    def forward(self, img, ground_truth = None):
        ...
        pred = inp
        out = inp
        out = torch.cat((out, feat[-1].unsqueeze(0)), dim=2) # 1, batch, 1024
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            result = self.cross_attention(img_channels, h0[-1]) # batch, 512 # h0[-1] is the last hidden state of the last layer
            result = result.unsqueeze(0) # 1, batch, 512
            out = torch.cat((out, result), dim=2) # 1, batch, 1024
