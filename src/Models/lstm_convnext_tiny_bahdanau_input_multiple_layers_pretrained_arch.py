...
class lstm_convnext_tiny_bahdanau_input_multiple_layers(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 2, return_attn = False, idx2word = None, use_pretrained_embed = False):
        super().__init__()
        self.DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.convnext_tiny = convnext_tiny(weights = 'IMAGENET1K_V1')
        
        if use_pretrained_embed:
            self.embed = AutoModelForCausalLM.from_pretrained("gpt2", cache_dir=pathlib.Path('cache').resolve())
        else: 
...
if __name__ == "__main__":
    ...
    # Init model
    model = lstm_convnext_tiny_bahdanau_input_multiple_layers(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)
