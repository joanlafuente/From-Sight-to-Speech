...
class lstm_bahdanau_multiple_layers_regions_convnext_features(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 2, return_attn = False, idx2word = None):
        ...
        self.h_att = nn.ModuleList([nn.Linear(768*2, 768).to(self.DEVICE) for i in range(rnn_layers)])
...
if __name__ == "__main__":
    ...
    # Init model
    model = lstm_bahdanau_multiple_layers_regions_convnext_features(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)

    img = torch.randn(2, 6, 768)
    
    import pickle as pkl
    
    with open('/fhome/gia07/Image_captioning/src/Dataset/features_new.pkl', 'rb') as f:
