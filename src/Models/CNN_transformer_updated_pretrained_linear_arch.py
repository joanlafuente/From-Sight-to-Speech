...
class CNN_transformer_updated_pretrained_linear(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0.1, transformer_heads=8, transformer_layers = 2, return_attn = False, idx2word = None, pretrained_embedding = False):
        ...
        # convexnet
        self.feature_extractor = convnext_tiny(weights = 'IMAGENET1K_V1')
                                      
        self.linear_enc = nn.Linear(768, dim_embedding)                 
    ...
    def forward(self, img, ground_truth = None):
        ...
        img_enc = self.feature_extractor.features(img) # batch, 768, 8, 8
        img_enc = self.feature_extractor.avgpool(img_enc).squeeze(-1).squeeze(-1).unsqueeze(1).permute(1, 0, 2) # 1, batch, 768
        
        img_enc = self.linear_enc(img_enc)
