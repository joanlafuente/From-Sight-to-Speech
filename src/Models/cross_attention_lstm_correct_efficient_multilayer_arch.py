...
class cross_attention_lstm_correct_efficient_multilayer(nn.Module):
    ...
    def forward(self, img, ground_truth = None):
        ...
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            for i in range(self.RNN.num_layers):
                h_attention = self.cross_attention(img_channels, h0[i]).unsqueeze(0) # batch, 512 # h0[-1] is the last hidden state of the last layer
