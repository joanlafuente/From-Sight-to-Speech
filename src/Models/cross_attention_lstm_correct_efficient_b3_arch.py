...
class cross_attention_lstm_correct_efficient_b3(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, idx2word, dropout = 0, rnn_layers = 1, return_attn = False):
        ...
        self.efficient = EfficientNetModel.from_pretrained('google/efficientnet-b3').to(self.DEVICE)
        ...
        self.cross_attention = CrossAttention(49, 1536, ret_attn=return_attn) # 
        self.hiddenattention2hidden = nn.Linear(1536, 1536)
        self.h_att = nn.Linear(1536*2, 1536)
        self.c_att = nn.Linear(1536*2, 1536)
    ...
    def forward(self, img, ground_truth = None):
        ...
        feat = self.efficient(img) # batch, 1536, 1, 1

        img_channels = feat.last_hidden_state # batch, 1536, 7, 7

        feat = feat.pooler_output.squeeze(-1).squeeze(-1).unsqueeze(0) # 1, batch, 1536 # .repeat(2, 1, 1) # Per utilitzar mes de una layer de la gru
        feat = feat.repeat(self.RNN.num_layers, 1, 1) # num_layers, batch, 1536
        start = torch.tensor(self.word2idx['<SOS>']).to(self.DEVICE)
        start_embed = self.embed(start) # 1536
        start_embeds = start_embed.repeat(batch_size, 1).unsqueeze(0) # 1, batch, 1536
        inp = start_embeds
        h0 = feat
        c0 = feat
        ...
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            out, (h0, c0) = self.RNN(out, (h0, c0)) # 1, batch, 1536
            ...
            if (ground_truth is not None) and (random.random() < self.teacher_forcing_ratio):
                ...
                out = self.embed(out).unsqueeze(0) # 1, batch, 1536
            else:
                out = out.permute(1, 0, 2) # batch, seq, 1536
                out = self.proj(out) # batch, seq,  NUM_WORDS
                out = out.permute(1, 0, 2) # seq, batch, NUM_WORDS
                _, out = out.max(2) # seq, batch
                out = self.embed(out) # seq, batch, 1536
            
            if self.return_attn:
                h0_att, weights_h0 = self.cross_attention(img_channels, h0[-1]) # batch, 1536 # h0[-1] is the last hidden state of the last layer
                h0_att = h0_att.unsqueeze(0)
                c0_att, weights_c0 = self.cross_attention(img_channels, c0[-1]) # batch, 1536 # c0[-1] is the last hidden state of the last layer
                ...
                h0_att = self.cross_attention(img_channels, h0[-1]).unsqueeze(0) # batch, 1536 # h0[-1] is the last hidden state of the last layer
                c0_att = self.cross_attention(img_channels, c0[-1]).unsqueeze(0) # batch, 1536 # c0[-1] is the last hidden state of the last layer
        ...
        res = pred.permute(1, 0, 2) # batch, seq, 1536
...
class CrossAttention(nn.Module):
    def __init__(self, input_size, hidden_size, ret_attn=False):
        super().__init__()
        self.fc_q = nn.Linear(hidden_size, hidden_size) # 1536 a 1536
    ...
    def scoringDot(self, keys, query):
        # query: batch, 1536
        # keys: batch, 1536, 1536
        query = torch.tanh(self.fc_q(query)) # batch, 49
        keys = torch.tanh(self.fc_k(keys))  # batch, 1536, 49
    ...
    def forward(self, channel_img, last_hidden_lstm):
        # channel_img: batch, 1536, 7, 7
        # output_lstm: batch, 1536
        channel_img = channel_img.view(channel_img.shape[0], channel_img.shape[1], -1) # Batch, 1536, 49

        values = torch.tanh(self.fc_v(channel_img)) # Batch, 1536, 1536
        weights = self.scoringDot(channel_img, last_hidden_lstm) # batch, 1, 49
        values = values.permute(0, 2, 1)
        result = torch.bmm(weights, values) # batch, 1, 1536
        result = result.squeeze(1) # batch, 1536
