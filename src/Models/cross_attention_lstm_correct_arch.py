...
class cross_attention_lstm_correct(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, dropout = 0, rnn_layers = 1, return_attn = False):
        super().__init__()
    ...
    def forward(self, img, ground_truth = None):
        ...
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            if self.return_attn:
                h0_att, weights_h0 = self.cross_attention(img_channels, h0[-1]) # batch, 512 # h0[-1] is the last hidden state of the last layer
                h0_att = h0_att.unsqueeze(0)
                c0_att, weights_c0 = self.cross_attention(img_channels, c0[-1]) # batch, 512 # c0[-1] is the last hidden state of the last layer
