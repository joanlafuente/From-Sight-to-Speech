...
class lstm_bahdanau_multiple_layers_regions_old_features(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 2, return_attn = False, idx2word = None):
        ...
        self.attention = BahdanauAttention(1280, rnn_layers, device=self.DEVICE)
        self.h_att = nn.ModuleList([nn.Linear(1280*2, 1280).to(self.DEVICE) for i in range(rnn_layers)])
    ...
    def forward(self, feat_regions, ground_truth = None):
        batch_size = feat_regions.shape[0]
        # feat_regions # batch, 6, 768
        feat_regions = feat_regions.permute(0, 2, 1) # batch, 768, 6
        
        # Get the regions that its features are not zero
        sum_reg = feat_regions.sum(dim=1) # batch, 6
        idx = sum_reg != 0
        mask = torch.zeros_like(sum_reg)
        idx = ~idx
        mask[idx] = float('-inf')
                
        feat = feat_regions[:, :, 0].unsqueeze(0) # 1, batch, 768 (This is a global representation of the image)
        ...
        start_embeds = start_embed.repeat(batch_size, 1).unsqueeze(0) # 1, batch, 768
        ...
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            weights = self.attention.score(h0, feat_regions, mask=mask) # batch, 1, 49
            attention_image = torch.bmm(weights, feat_regions.permute(0, 2, 1)).permute(1, 0, 2) # batch, 1, 512
...
class BahdanauAttention(nn.Module):
    ...
    def score(self, hidden, encoder_output, mask = None):
        ...
        if mask is not None:
            out_attn = out_attn + mask.unsqueeze(1)
...
if __name__ == "__main__":
    ...
    # Init model
    model = lstm_bahdanau_multiple_layers_regions_old_features(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)

    img = torch.randn(2, 6, 768)
    
    import pickle as pkl
    
    with open('/fhome/gia07/Image_captioning/src/Dataset/features.pkl', 'rb') as f:
        dict_features = pkl.load(f)
    print(torch.tensor(dict_features["444047125_66b249287c.jpg"]).unsqueeze(0).size())

    res = model(torch.tensor(dict_features["444047125_66b249287c.jpg"]).unsqueeze(0))
