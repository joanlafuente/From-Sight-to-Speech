...
class lstm_convnext_tiny_bahdanau_hidden_pretrained_embed(nn.Module):
    def __init__(self, text_max_len, teacher_forcing_ratio, NUM_WORDS, word2idx, dropout = 0, rnn_layers = 2, return_attn = False, idx2word = None):
        ...
        pretrained_embed = gensim.models.KeyedVectors.load_word2vec_format('/fhome/gia07/Image_captioning/src/Dataset/glove.6B.300d.txt', binary=False, no_header=True)
        ...
        for i in range(NUM_WORDS):
            ...
            if word in pretrained_embed:
                embeddings[word2idx[word], :] = torch.Tensor(pretrained_embed[word])
                counter += 1
        ...
        self.embed_proj = nn.Linear(300, 768)
    ...
    def forward(self, img, ground_truth = None):
        ...
        feat = feat.squeeze(-1).squeeze(-1).unsqueeze(0) # 1, batch, 768
        feat = feat.repeat(self.RNN.num_layers, 1, 1) # num_layers, batch, 768
        start = torch.tensor(self.word2idx['<SOS>']).to(self.DEVICE)
        start_embed = self.embed(start) # 768
        start_embed = self.embed_proj(start_embed) # 768
        ...
        for i in range(self.TOTAL_MAX_WORDS-1): # rm <SOS>
            ...
            out, (h0, c0) = self.RNN(out, (h0, c0)) # 1, batch, 768
            ...
            if (ground_truth is not None) and (random.random() < self.teacher_forcing_ratio):
                ...
                out = self.embed(out).unsqueeze(0) # 1, batch, 300
                out = self.embed_proj(out) # 1, batch, 768
            else:
                ...
                out = self.embed(out) # seq, batch, 300
                out = self.embed_proj(out) # seq, batch, 768
            
            weights = self.attention.score(h0, img_channels) # batch, 1, 49
            attention_image = torch.bmm(weights, img_channels.permute(0, 2, 1)).permute(1, 0, 2) # batch, 1, 768
        ...
        if not self.return_attn:
            res = pred.permute(1, 0, 2) # batch, seq, 768
...
if __name__ == "__main__":
    ...
    # Init model
    model = lstm_convnext_tiny_bahdanau_hidden_pretrained_embed(text_max_len=38, teacher_forcing_ratio=0, NUM_WORDS=NUM_WORDS, word2idx=word2idx)
