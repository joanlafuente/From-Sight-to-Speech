...
def get_grad_cam(img, output, model, loss, i=1):

    # Zero the gradients of the model
    model.zero_grad()

    _, pred = torch.max(output, dim=1)
    loss = loss(output[0, :, i], pred[0, i])
    ...
    # Get the last feature map of the model
    gradients = model.convnext_tiny.features()

    print(gradients.shape)
    print(activations.shape)
    importance_weights = np.mean(gradients * activations, axis=(1, 2))

    # Compute the weighted average of the channels
    ...
    cam = F.interpolate(torch.tensor(cam).unsqueeze(0).unsqueeze(0), size=(224, 224), mode='bilinear').view(224, 224).numpy()
    return cam
...
for e, (img, caption) in enumerate(tqdm(dataloader_test)):
    ...
    axs[0].set_title("Original image")
    axs[0].axis("off")
    for i in tqdm(range(len(list_words_plot))):
        # Get the grad cam for the i-th word
        grad_cam_img = get_grad_cam(img, outputs, model, crit, i=i+1)
        
        axs[i+1].imshow(img2plot)
        axs[i+1].imshow(grad_cam_img, alpha=0.35, cmap='jet')
        axs[i+1].set_title(list_words_plot[i])
        axs[i+1].axis("off")

    plt.axis("off")
    plt.show()
    plt.savefig(f'grad_cam_imgid{e}.png', dpi=300)
    plt.close()
    if input() == 'q':
        break
