...
from Models.cross_attention_lstm_correct_arch import cross_attention_lstm_correct
...
model = cross_attention_lstm_correct(text_max_len=38, teacher_forcing_ratio=0, rnn_layers=1, return_attn=True).to(device)

metric = metrics_evaluation

# model.load_state_dict(torch.load(f'/fhome/gia07/Image_captioning/src/runs-baseline/attention_corrected/weights/123.pth', map_location=device))

model.eval()

dataset_train = Data_word_visuals(data, partitions['train'], train=False)
dataloader_train = torch.utils.data.DataLoader(dataset_train, batch_size= 1, shuffle= True)
# Val
dataset_valid = Data_word_visuals(data, partitions['valid'], train=False)
dataloader_valid = torch.utils.data.DataLoader(dataset_valid, batch_size= 1, shuffle= True)
# Test
dataset_test = Data_word_visuals(data, partitions['test'], train=False)
...
for img, caption in tqdm(dataloader_test):
    ...
    fig, axs = plt.subplots(1, len(list_words_plot), figsize=(30, 10))
