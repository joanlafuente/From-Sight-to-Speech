...
with open('/fhome/gia07/Image_captioning/src/runs-baseline/VIT_pretrained_emb_lowerlr_5cap_evenSimpler_harder_frozen/word2idx.json', 'r') as f:
...
model = VIT_transformer_updated_pretrained_linear(text_max_len=40, NUM_WORDS=NUM_WORDS, teacher_forcing_ratio=0, word2idx=word2idx, transformer_heads=12, transformer_layers=5, dropout=0, pretrained_embedding = False, return_attn = True).to(device)
...
outputs, input_words = model(img)

_, preds = torch.max(outputs, dim=1)
preds = preds.cpu().numpy()

pred_text = ' '.join([idx2word[word] for word in preds[0]])

import wandb
wandb.init(project="Image_Captioning2", name="Attention_VIT_pretrained_emb_lowerlr_5cap_evenSimpler_harder_frozen")

upload_self_attention(save_output, input_words, idx2word)
